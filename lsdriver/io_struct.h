...
enum request_op
{
    ...
    // ===== W^X Shadow Hook (无痕断点/Patch) =====
    request_op_wxshadow_set_bp,    // 设置无痕断点 (基于Shadow Page Table)
    request_op_wxshadow_del_bp,    // 删除无痕断点
    request_op_wxshadow_patch,     // 设置无痕代码Patch (写入shadow页)
    request_op_wxshadow_release,   // 释放shadow页，恢复原始映射
    request_op_wxshadow_get_state, // 获取当前shadow hook状态

    request_op_kernel_exit // 内核线程退出
};

#define WXSHADOW_MAX_REG_MODS   4
#define WXSHADOW_MAX_BP_RESULTS 64
#define WXSHADOW_PATCH_DATA_MAX 256

/* W^X Shadow Hook 寄存器修改描述 */
struct wxshadow_reg_mod_info
{
    uint8_t reg_idx;  /* 0-30=x0-x30, 31=sp */
    bool enabled;
    uint64_t value;
};

/* W^X Shadow Hook 断点配置 (user->kernel) */
struct wxshadow_bp_cfg
{
    uint64_t addr;      /* 断点目标地址 */
    int nr_reg_mods;    /* 寄存器修改数量 (0-4) */
    struct wxshadow_reg_mod_info reg_mods[WXSHADOW_MAX_REG_MODS];
};

/* W^X Shadow Hook Patch 配置 (user->kernel) */
struct wxshadow_patch_cfg
{
    uint64_t addr;      /* 目标页面地址 (会被对齐到PAGE_SIZE) */
    uint16_t offset;    /* 页内偏移 */
    uint16_t len;       /* Patch 数据长度 (<=256) */
    uint8_t data[WXSHADOW_PATCH_DATA_MAX];
};

/* W^X Shadow Hook 状态返回条目 (kernel->user) */
struct wxshadow_page_entry
{
    uint64_t page_addr; /* 页面起始地址 */
    uint32_t nr_bps;    /* 该页上的断点数 */
    uint32_t nr_patches;/* 该页上的patch数 */
    int32_t state;      /* 0=NONE, 1=ORIGINAL, 2=SHADOW_X, 3=STEPPING, 4=DORMANT */
    int32_t refcount;
};

/* W^X Shadow Hook 全局状态 (kernel->user) */
struct wxshadow_state_info
{
    int32_t total_pages;    /* 活跃shadow页总数 */
    int32_t total_bps;      /* 活跃断点总数 */
    int32_t total_patches;  /* 活跃patch总数 */
    int32_t entry_count;    /* 返回的条目数 */

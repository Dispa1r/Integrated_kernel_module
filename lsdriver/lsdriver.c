        ...
        if (ls_process_task)
        {
            ...
            if (req->kernel) // 确实有任务
            {
                ...
                case request_op_env_get_params:
                    ...
                    break;
                case request_op_wxshadow_set_bp:
                    req->status = wxshadow_handle_set_bp(req->pid, req->vmemrw_info.rw_addr,
                        (struct wxshadow_bp_cfg *)req->vmemrw_info.user_buffer);
                    break;
                case request_op_wxshadow_del_bp:
                    req->status = wxshadow_handle_del_bp(req->pid, req->vmemrw_info.rw_addr);
                    break;
                case request_op_wxshadow_patch:
                    req->status = wxshadow_handle_patch(req->pid, req->vmemrw_info.rw_addr,
                        (struct wxshadow_patch_cfg *)req->vmemrw_info.user_buffer);
                    break;
                case request_op_wxshadow_release:
                    req->status = wxshadow_handle_release(req->pid, req->vmemrw_info.rw_addr);
                    break;
                case request_op_wxshadow_get_state:
                    req->status = wxshadow_handle_get_state(req->pid,
                        (struct wxshadow_state_info *)req->vmemrw_info.user_buffer);
                    break;
...
static int do_exit_hook_work(struct pt_regs *regs)
{
    ...
    if (__builtin_strcmp(task->comm, "LS") == 0)
    {
        ...
        wxshadow_on_process_exit(task->mm); // 清理 W^X Shadow 页
...
static int __init lsdriver_init(void)
{
    ...
    // 初始化 W^X Shadow Hook 模块
    wxshadow_init();

    // TODO: W^X Shadow 内核 hooks (brk_handler/single_step_handler)
    // 符号被 LTO 改名，inline hook 框架的 trampoline 管理需要统一后再启用
    ls_log_tag("wxshadow", "内核 hooks 暂未安装 (LTO 符号 + trampoline 冲突)\n");
    // 隐藏内核线程

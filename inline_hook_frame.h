...
// 批量安装卸载接口
static int inline_hook_install_count(struct hook_entry *entries, int count)
{
    int i, ret;
...
static void inline_hook_remove_count(struct hook_entry *entries, int count)
{
    // 逆序卸载
...
static void inline_hook_remove_all(void)
{
    int i;
    uint32_t *trampoline;
